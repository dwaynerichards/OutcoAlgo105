...
assert(
    testCount,
    'should return true for a word that exists within larger word',
    () => {
        let trie = new Trie();
        trie.insert('cat');
        trie.insert('cats');
...
assert(
    testCount,
    'should return false if a smaller word was not added, but exists in a larger word',
    () => {
        let trie = new Trie();
        trie.insert('cats');
...
assert(testCount, 'has startsWith method', () => {
    let trie = new Trie();
    return (
        Object.prototype.toString.apply(trie.startsWith) === '[object Function]'
    );
});
...
assert(
    testCount,
    'returns correct prefixes including input that is a word',
    () => {
        let trie = new Trie();
        trie.insert('cat');
        trie.insert('cats');
        ...
        return (
            results !== undefined && arraysMatching(results, ['car', 'cars'])
        );
    }
);
...
assert(testCount, 'returns the correct prefixes', () => {
    ...
    return (
        results !== undefined &&
        arraysMatching(results, ['cat', 'cats', 'catnip', 'car', 'cars'])
    );
});
...
assert(
    testCount,
    'does not remove letters that belong to a longer word',
    () => {
        let trie = new Trie();
        trie.insert('hello');
        trie.insert('hell');
...
assert(
    testCount,
    'removes letters from longer word and keeps shorter letters',
    () => {
        let trie = new Trie();
        trie.insert('cat');
        trie.insert('cats');
