...
class Matrix {
    ...
    initialize(arrayOfArrays: number[][]) {
        //mutate storage
    ...
    insert(i: number, j: number, val: number) {
    ...
    scale(factor: number) {
        for (let i = 0; i < this.m; i++) {
            for (let j = 0; j < this.n; j++) {
                const val = this.retrieve(i, j);
                const scaled = factor * val;
                this.insert(i, j, scaled);
            }
        }
        /**
 * highOrderFunctionVersion
 ...
 */
    ...
    flatten() {
        ...
        for (let i = 0; i < this.m; i++) {
            for (let j = 0; j < this.n; j++) {
                results.push[this.retrieve(i, j)];
            }
        }
    ...
    slice(rowRange: number[], colRange: number[]): Array<number[]> | null {
        //YOUR WORK HERE
        ...
        type rangeCheck = (range: number[], standard: number) => boolean;
        const inBounds: rangeCheck = (range, bound) => {
            return range[0] >= bound && range[0] < range[1] && range[1] < bound;
        };
        if (!inBounds(rowRange, this.m) || !inBounds(colRange, this.n))
            return null;
        const storage: Array<number[]> = [];
        this.storage = this.storage.reduce((accum, row, i) => {
            if (inBounds(rowRange, i))
                //if row index is inbounds rowRange
                accum.push(row.slice(colRange[0], colRange[1])); //copy colrages into accumulator
            return accum;
        }, storage);
        return this.storage;
    }
 ...
 */
    ...
    multiply(matrix: Matrix) {
        //YOUR WORK HERE
        const newMatrix = new Matrix(this.m, matrix.n);
        for (let i = 0; i < newMatrix.m; i++) {
            for (let j = 0; j < newMatrix.n; j++) {
                let val = 0;
                for (let k = 0; k < this.n; k++) {
                    val += this.retrieve(i, k) * matrix.retrieve(k, j);
                }
                newMatrix.insert(i, j, val);
            }
        }
        return newMatrix;
