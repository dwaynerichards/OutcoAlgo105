      ...
      const reInsert = (bucket) => {
        ...
        if (bucket[0] !== undefined) {
          ...
          for (const array of bucket) {
            ...
            let value = array[1];
            console.log(
              `this is from iteration of arrs in each bucket`,
              key,
              value,
              this.loadFactor(),
              this.size,
...
assert(testCount, "can insert a second key-value pair into hashtable", () => {
  ...
  return hashTable.size === 2 && Array.isArray(hashTable.storage[index1]) && Array.isArray(hashTable.storage[index2]);
});
...
assert(testCount, "does not remove a key-value pair that does not exist", () => {
  ...
  let index = hashTable.hash("hello", hashTable.buckets);
  return (
    hashTable.size === 1 && hashTable.storage[index][0][0] === "hello" && hashTable.storage[index][0][1] === "world"
  );
});
...
assert(
  testCount,
  "halves buckets if load factor drops equal to or below " + "25% and bucket length is greater than 8",
  () => {
    let hashTable = new HashTable();
    const keys = ["zero", "one", "two", "three", "four", "five", "six"];
    const values = ["0", "1", "2", "3", "4", "5", "6"];
