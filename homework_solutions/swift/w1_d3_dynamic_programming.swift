 ...
 *					The most optimal solution of a larger problem can be solved using
 *					optimal solutions of its subproblems. "optimal" solution means the
 *					most accurate or best solution.
 */
 ...
 */
 ...
 func latticePaths(_ n: Int, _ m: Int) -> Int {
   ...
   let larger: Int = max(n, m)
   let smaller: Int = min(n, m)

   while work.count < larger + 1 {
     for i in 1..<work.count {
       work[i] += work[i - 1]
     }
     work.append(2 * work[work.count - 1])
   }
   return  work[smaller]
 }
