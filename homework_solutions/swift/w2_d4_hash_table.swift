...
class HashTable {
 ...
 func insert(_ key: String, _ value: String) {
   ...
   for (i, pair) in storage[index].enumerated() {
     if pair[0] == key {
 ...
 func resize() {
   let loadFactor = Double(size) / Double(buckets)
   if loadFactor > 0.25 && loadFactor < 0.75 {
     return
   }
   if loadFactor <= 0.25 && buckets == 8 {
     return
   }
   let temp = storage
   if loadFactor >= 0.75 {
   ...
   storage = Array(repeating: [], count: self.buckets)
   for bucket in temp {
     for pair in bucket {
       size -= 1
