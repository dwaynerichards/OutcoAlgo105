...
class Graph {
  ...
  func addEdge(_ id1: Int,_ id2: Int) -> Bool {
    if storage[id1] == nil || storage[id2] == nil {
      return false
    }
    if (storage[id1]?.contains(id2))! {
      return false
    }
  ...
  func removeEdge(_ id1: Int, _ id2: Int) -> Bool {
    if(storage[id1] != nil) {
      let index = storage[id1]?.firstIndex(of: id2)
      if(index != nil){
        storage[id1]?.remove(at: index!)
        return true
      }
    }
    return false
  ...
  func isVertex(_ id: Int) -> Bool {
    return storage[id] == nil ? false : true
}

  // Time Complexity: O(1)
  // Auxiliary Space Complexity: O(1)
  func neighbors(_ id: Int) -> [Int]? {
    return storage[id] == nil ? nil : storage[id]
  }
}
