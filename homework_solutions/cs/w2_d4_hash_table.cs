...
class HashTable {
  ...
  public void insert(string key, string value) {
    ...
    if (storage[index] == null) {
      this.storage[index] = new List<string[]>();
    }
  ...
  public void resize() {
    float loadFactor = this.size / (float)buckets;
    if (loadFactor > 0.25 && loadFactor < 0.75) { return; }
    if (loadFactor <= 0.25 && buckets == 8) { return; }

    List<string[]>[] temp = this.storage;
    buckets = loadFactor >= 0.75 ? buckets * 2 : buckets / 2;
    this.storage = new List<string[]>[buckets];
