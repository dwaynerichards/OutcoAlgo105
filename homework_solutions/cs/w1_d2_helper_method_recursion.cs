...
class ReverseString {
    ...
    private static string buildString(string input, string output) {
        if (input.Length == 0) {
            return output;
        }
        output = input[0]+output;
        return buildString(input.Substring(1), output);
    }
}
...
class ArrayPairs {
    ...
    public static int[,] compute(int[] arr) {
        int numPairs = (arr.Length % 2 == 0 ? arr.Length / 2 : arr.Length / 2 + 1);
...
class Flatten {
    ...
    private static void assemble(int[,] matrix, int subIndex, int resultIndex, int[] result) {
        if (subIndex >= matrix.GetLength(0)) {
            return;
        }
