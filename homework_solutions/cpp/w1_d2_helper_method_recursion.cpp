...
class ReverseString {
    ...
    private:
      string buildString(string input, string output) {
        if (input.size() == 0) {
            return output;
        }
        output = input[0]+output;
        return buildString(input.substr(1), output);
      }
};
...
class ArrayPairs {
    ...
    public:
      vector<vector<int>> compute(vector<int> arr) {
        int numPairs = (arr.size() % 2 == 0 ? arr.size() / 2 : arr.size() / 2 + 1);
        vector<vector<int>> result(numPairs, vector<int>(2,0));
        makePair(arr, 0, result);
        return result;
      }

    private:
      void makePair(vector<int> &arr, int index, vector<vector<int>> &result) {
        if (index >= arr.size()) {
            return;
        }
...
class Flatten {
    ...
    private:
      void assemble(vector<vector<int>> &matrix, int subIndex, int resultIndex, vector<int> &result) {
        if (subIndex >= matrix.size()) {
            return;
        }

        for (int i = 0 ; i < matrix[subIndex].size() ; i++) {
...
class Power {
    ...
    private:
      int multiply(int numBase, int exponent, int result) {
        if (exponent == 0) return result;
        return multiply(numBase, exponent - 1, result) * numBase;
      }
};
...
class Merge {
  ...
  public:
    vector<int> compute(vector<int> arr1, vector<int> arr2) {
      totalElements = arr1.size() + arr2.size();
  ...
  private:
    ...
    void compare(vector<int> &result, vector<int> &arr1, vector<int> &arr2, int i, int j) {
