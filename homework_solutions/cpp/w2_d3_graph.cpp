  ...
  public:
    ...
    bool removeVertex(int id) {
      ...
      for(auto storagePair: storage) {
        vector<int> edges = storagePair.second;
        vector<int>::iterator itr = std::find(edges.begin(), edges.end(), id);
        if(itr != edges.cend()) {
          int index = distance(edges.begin(), itr);
    ...
    bool addEdge(int id1, int id2) {
      if(storage.find(id1) == storage.end() || storage.find(id2) == storage.end()) {
        return false;
      }

      if(count(storage[id1].begin(), storage[id1].end(), id2)) {
        return false;
      }
      storage[id1].push_back(id2);
      return true;
    }
    ...
    bool removeEdge(int id1, int id2) {
      if (storage.find(id1) != storage.end() && storage.find(id2) != storage.end()) {
        vector<int>::iterator id1Iterator = find(storage[id1].begin(), storage[id1].end(), id2);

        if(id1Iterator != storage[id1].cend()) {
          int index = distance(storage[id1].begin(), id1Iterator);
          storage[id1].erase(storage[id1].begin() + index);
          return true;
        }
      }
      return false;
    ...
    vector<int> neighbors(int id) {
      return storage.find(id) != storage.end() ? storage[id] : vector<int>();
    }
