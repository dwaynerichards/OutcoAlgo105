...
class Trie {
    ...
    public:
      ...
      void remove(string word) {
        ...
        while(!current->end && stack.size() > 0) {
          ...
          current = stack.top();
          stack.pop();
          current->children.erase(previousLetter);
        }
    ...
    private:
      ...
      vector<string> _traverseNode(TrieNode *node) {
        vector<string> results;
        dfs(node, "", results);
        return results;
      }

      void dfs(TrieNode *current, string path, vector<string> &results) {
        if(current == NULL) return;
        if(current->end) {
        ...
        char childKey;
        TrieNode *childNode;
        for(auto childrenPair: current->children) {
          childKey = childrenPair.first;
          childNode = childrenPair.second;
          dfs(childNode, path + childKey,results);
        }
      }
};
