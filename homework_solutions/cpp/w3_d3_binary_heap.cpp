...
class Heap {
  public:
    ...
    void bubbleUp(int index){
      ...
      // integer division floors result, working for both even and odd childindex
    ...
    int removePeak(){
      swap(0, this->size() - 1);
      int toReturn = storage[this->size()-1];
      storage.erase(storage.begin() + size()-1);
      bubbleDown(0);
      return toReturn;
    }
    ...
    int _getChild(int parentIndex) {
      int childIndex1 = 2 * parentIndex + 1;
      int childIndex2 = 2 * parentIndex + 2;
      if (childIndex1 >= this->size() || childIndex2 > this->size()) {
        return childIndex1;
      } else {
    ...
    bool remove(int value){
      for (int i = 0 ; i < this->size() ; i++) {
        if (storage[i] == value) {
          swap(i, this->size() - 1);
          storage.erase(storage.begin() + this->size()-1);
          bubbleUp(i);
          bubbleDown(i);
          return true;
        }
