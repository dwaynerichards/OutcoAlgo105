...
func closestValue(arr []int, target int) int {
  ...
  value := arr[start + (end - start)/2]
  var midpoint, difference int
  for start <= end {
    midpoint = start + (end - start)/2
    difference = int(math.Abs(float64(target - arr[midpoint])))
...
func squareRoot(n float64) float64 {
  ...
  var low,high,result,sq,mid float64 = 0.0, n, 0.0, 0.0, 0.0
  for low <= high {
    mid = (low + high) / 2
    sq = mid * mid
    if sq == n {
...
func rotatedArraySearch(nums []int, target int) bool {
  ...
  if target >= nums[0] {
    ...
    return binarySearch(nums, end + 1, len(nums) - 1, target)
  }
}
