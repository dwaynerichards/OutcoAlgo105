...
class HashTable
  ...
  def resize()
    load_factor = @size / @buckets.to_f
    return if load_factor > 0.25 and load_factor < 0.75
    return if load_factor <= 0.25 and @buckets == 8

    temp = @storage
    @buckets = load_factor >= 0.75 ? @buckets * 2 : @buckets / 2
    @storage = []
    temp.each do |bucket|
      if bucket != nil
        bucket.each do |pair|
