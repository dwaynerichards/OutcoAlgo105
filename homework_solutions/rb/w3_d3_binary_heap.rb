...
class Heap
  ...
  def bubble_up(index)
    ...
    while (child_index > 0 && !compare(parent_index, child_index))
  ...
  def bubble_down(index)
    ...
    if (child_index1 >= size())
      return
    elsif (child_index2 >= size())
      master_child_index = child_index1
    elsif (compare(child_index1, child_index2))
      master_child_index = child_index1
    else
      master_child_index = child_index2
    end

    while (parent_index < size() && !compare(parent_index, master_child_index))
  ...
  def remove(value)
    ...
    return value.to_s + ' does not exist'
  end
