...
def power(base, exponent)
  ...
  multiply = lambda do |counter|
    if (counter == exponent)
      return
    end
    results = results * base
    multiply.call(counter + 1)
  end
...
def merge(arr1, arr2)
  results = []
  total_elements = arr1.length + arr2.length
  compare = lambda do |i, j|
