...
string minimumWindowSubstring(string str, string targets) {
   int result[] = {0, INT_MAX};
   unordered_map<char, int> counts;
   int missingchars = targets.length();
   ...
   for (int fast = 0; fast < str.length(); fast++) {
     if (counts.find(str[fast]) != counts.end()) {
     ...
     while (missingchars == 0) {
       ...
       if (counts.find(str[slow]) != counts.end()) {
         counts[str[slow]]--;
   ...
   return result[1] == INT_MAX ? "" : str.substr(result[0], result[1]-result[0] + 1);
 }
...
int escape(vector<vector<int>> dungeon) {
  int yBound = dungeon.size();
  int xBound = dungeon[0].size();
  ...
  for (int y = yBound - 2; y > -1; y--) {
    for (int x = xBound - 2; x > -1; x--) {
      int fromRight = max(dungeon[y][x+1] - dungeon[y][x], 1);
      int fromBelow = max(dungeon[y+1][x] - dungeon[y][x], 1);
