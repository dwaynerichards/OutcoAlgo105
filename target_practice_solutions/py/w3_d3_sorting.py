...
#  Problem 1: Quicksort
#
#  Prompt:    Given an unsorted list of integers, return the list sorted using
#             quicksort.
#
    ...
    def divide(start, finish):
        ...
        for i in range(start, finish):
            if input[i] < input[pivot]:
                input[i], input[wall] = input[wall], input[i]
                wall += 1

        input[wall], input[pivot] = input[pivot], input[wall]
        divide(start, wall - 1)
        divide(wall + 1, finish)
