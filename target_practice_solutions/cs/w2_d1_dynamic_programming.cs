...
class Problems {
  ...
  // Auxiliary Space Complexity: O(N)
   public static string minimumWindowSubstring(string str, string targets) {
     ...
     for (int fast = 0; fast < str.Length; fast++) {
       if (counts.ContainsKey(str[fast])) {
       ...
       while (missingchars == 0) {
         ...
         if (counts.ContainsKey(str[slow])) {
           counts[str[slow]]--;
     ...
     return result[1] == int.MaxValue ? "" : str.Substring(result[0], result[1]-result[0] + 1);
   }
  ...
  */
  ...
  public static int escape(int[,] dungeon) {
    int yBound = dungeon.GetLength(0);
    int xBound = dungeon.GetLength(1);
