...
func powerSet(_ str: String) -> [String] {
    var result = [String]()
    let letters = Array(str.characters)

    func traverse(build: String, depth: Int) {

        if depth == str.characters.count {
            result.append(build)
            return
        }
        traverse(build: build, depth: depth + 1)
        traverse(build: build + String(letters[depth]), depth: depth + 1)
    }

    traverse(build: "", depth: 0)
    return result
}
